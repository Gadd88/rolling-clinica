...
document.addEventListener("DOMContentLoaded", () => {
  if (usuarioSession && (usuarioSession != null || usuarioSession != undefined)) {
    btnRegLogSection.forEach(
      section => {
        section.innerHTML = `
            <div class="d-flex flex-column flex-lg-row align-items-center gap-lg-5 me-5">
                <h4 class="text-dark fs-5">${usuarioSession.length>0 ? usuarioSession[0].usuario.split(" ")[0] : usuarioSession.usuario.split(" ")[0]}</h4>
...
btnLoginIngresar.addEventListener("click", (e) => {
  ...
  if(usuarioFiltrado.length>0){
    ...
    Swal.fire({
      icon: "success",
      title: "Bienvenido",
      text: `${usuarioFiltrado[0].usuario}`,
      timer: 1000,
      });
  }else if (
    JSON.stringify(usuarioLoggeado.usuario) ===
    JSON.stringify(paciente.usuario) ||
    JSON.stringify(usuarioLoggeado) === JSON.stringify(doctor) ||
    JSON.stringify(usuarioLoggeado) === JSON.stringify(admin)
  ) {
    userLogged = true;
