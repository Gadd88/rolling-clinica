...
const mostrarTurnos = () => {
  ...
  if (turnos.length < 1)
    return (listaTurnos.innerHTML = "<h3>No hay turnos agendados</h3>");
  turnos.forEach((turno) => {
    const { paciente, especialidad, fechaTurno, horaTurno, id } = turno;
    listaTurnos.innerHTML += `<li class="card" style="width: 18rem;" id=${id}>
            <div class="card-body">
                ...
                <h5 class="card-subtitle mb-2 text-body-secondary">Fecha: ${new Date(
                  fechaTurno
                ).toLocaleDateString("es-AR")} </h5>
...
listaTurnos.addEventListener("click", (e) => {
  if (
    e.target.classList.contains("eliminarTurno") ||
    e.target.parentElement.classList.contains("eliminarTurno")
  ) {
